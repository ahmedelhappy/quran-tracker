...
| **Implementation session #1** (2026-07-03) | Executed Stages 0 → 1.8a (mushaf rendering, reader UX rounds, bookmarks, streak fix, mode merge, scrubber) | Retired after Stage 1.8a — context window full. Commits 153dd3b … the 1.8a push |
| **Implementation session #2** (next) | Will start with Stage 1.8b (margin ornaments) or Stage 2 (Quran data rebuild) | Prompts are self-contained in this file |
...
| 2026-07-03 | Stage 1.8a — unified reader | **accepted** | Mode merge, scrubber (PageScrubber.jsx, repositioned directly under the page card), badge cleanup, CODE_GUIDE §6 rewrite — 34/34 server tests, lint/build clean. (The "missing" method checklist was a stale-HMR ghost — it was in the code all along; dev-server restart resolved it.) Commit + push instructed |
| 2026-07-03 | Stage 1.8b — mushaf margin ornaments | — | Added to plan: juz / hizb / quarter-hizb boundary marks + sajda (۩) marks in the page margins, like the printed Madinah mushaf (data already in the quran.com verse fields; waqf/saktah/sajda-overline are already in the QCF glyphs; ruku'/manzil deliberately skipped) |
