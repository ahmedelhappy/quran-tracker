// The 240 rub-el-hizb (quarter-hizb) start verses, in reading order.
//
// This is the SAME list the backend derives its hizb/rub verse ranges from (see
// server/utils/segments.js), generated from the same committed
// server/seed/data/quranStructure.json — so a unit means exactly the same span
// of verses on both sides, and the two can never drift. A mushaf page carries at
// most one rub boundary, so concatenating them in page order is already reading
// order. Regenerate the same way if that file ever changes.
//
// Rub n spans from RUB_BOUNDARIES[n - 1] to the verse before RUB_BOUNDARIES[n]
// (or the end of the Quran for rub 240); a hizb is four consecutive rubs.

export const RUB_BOUNDARIES = [
