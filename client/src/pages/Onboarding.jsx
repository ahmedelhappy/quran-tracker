      ...
      <main className="flex-1 w-full max-w-[800px] mx-auto px-6 pb-12 flex flex-col gap-10">
        ...
        <section className="flex flex-col gap-4">
          <div>
            ...
            <p className="text-[#404944]">Consistency is key. Choose a manageable daily pace.</p>
          </div>
            ...
            {[
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center gap-2 p-6 rounded-xl border-2 transition-colors relative overflow-hidden ${
                  dailyPages === value
                    ? 'border-[#003527] bg-[#b0f0d6] text-[#064e3b]'
                    : 'border-[#bfc9c3] bg-[#f9f9ff] text-[#404944] hover:bg-[#e7eefe]'
                }`}
              >
        ...
        {/* Estimate banner */}
        {estimatedYears && (
          <section className="bg-[#f0f3ff] rounded-xl p-6 border border-[#bfc9c3]/50 flex items-start md:items-center gap-6 flex-col md:flex-row">
            ...
            <div>
              <h3 className="text-xl font-semibold text-[#151c27] mb-1">Estimated Completion</h3>
              <p className="text-[#404944]">
                At a pace of{' '}
                <strong className="text-[#151c27]">{dailyPages} page{dailyPages !== 1 ? 's' : ''} per day</strong>,
                you will complete the remaining {Math.round(remaining / 20)} Juz in approximately{' '}
                <strong className="text-[#003527]">{estimatedYears} year{estimatedYears !== 1 ? 's' : ''}</strong>.
              </p>
            </div>
          </section>
        )}
    ...
    <div className="min-h-screen bg-[#f9f9ff] sacred-pattern flex flex-col">
      ...
      <main className="flex-1 w-full max-w-[800px] mx-auto px-6 pb-12 flex flex-col gap-8">
        <div className="text-center mb-4">
          <h2 className="text-2xl font-semibold text-[#151c27] mb-2">Customize Your Experience</h2>
          <p className="text-[#404944]">Set a sustainable rhythm. You can always adjust this later.</p>
        </div>

        {/* Review Intensity */}
        <div className="bg-white rounded-xl sacred-shadow border border-[#dce2f3] p-6">
          <h3 className="font-semibold text-[#151c27] mb-1">Review Intensity</h3>
              ...
              <label key={value} className="cursor-pointer">
                ...
                <div className={`p-4 rounded-xl border-2 transition-all h-full ${
                  ...
                  <div className="flex justify-between items-center mb-2">
                    <span className={`font-medium ${reviewIntensity === value ? 'text-[#904d00]' : 'text-[#151c27]'}`}>{label}</span>
                    <span className={`text-sm ${reviewIntensity === value ? 'text-[#fe932c]' : 'text-[#bfc9c3]'}`}>
                      {reviewIntensity === value ? '●' : '○'}
                    </span>
                  </div>
        ...
        {/* Rest Days */}
        <div className="bg-white rounded-xl sacred-shadow border border-[#dce2f3] p-6">
          <h3 className="font-semibold text-[#151c27] mb-1">Rest Days</h3>
          <p className="text-sm text-[#404944] mb-4">Select up to 2 days where no new memorization will be scheduled.</p>
          ...
          {offDays.length === 2 && (
            <p className="text-xs text-amber-600 mt-2">Maximum 2 rest days selected</p>
          )}
        </div>
    ...
    <div className="min-h-screen bg-[#f9f9ff] sacred-pattern flex flex-col">
      ...
      <main className="flex-1 w-full max-w-[800px] mx-auto px-6 pb-12 flex flex-col gap-8">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-semibold text-[#151c27]">Your Plan is Ready! ✨</h1>
          <p className="text-sm text-[#404944] mt-2 max-w-md mx-auto">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[#003527] rounded-2xl p-5 text-white">
            ...
            <p className="text-4xl font-bold">{estimatedYears ? `${Math.round((estimatedDays ?? 0) / 30)}` : '—'}</p>
            <p className="text-sm text-[#80bea6]">Months</p>
          </div>
          <div className="bg-white rounded-2xl border border-[#dce2f3] p-5 sacred-shadow">
            <p className="text-xs font-semibold uppercase tracking-wide text-[#404944] mb-1">Target Goal</p>
            <p className="text-3xl font-bold text-[#151c27]">Entire</p>
            <p className="text-sm text-[#404944]">Quran (604 pages)</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={label} className="bg-white rounded-xl border border-[#dce2f3] p-4 text-center sacred-shadow">
              <p className="text-xs text-[#404944] mb-1">{label}</p>
              <p className="text-sm font-bold text-[#064e3b]">{value}</p>
            </div>
          ))}
        </div>

        <div className="bg-white rounded-xl border border-[#dce2f3] p-6 sacred-shadow">
          <h3 className="font-semibold text-[#151c27] mb-4">Tips for Success</h3>
          <div className="grid sm:grid-cols-2 gap-3">
            {[
              ...
              <div key={title} className="flex gap-3 p-3 bg-[#f9f9ff] rounded-xl">
                <span className="text-xl flex-shrink-0">{icon}</span>
                <div>
                  <p className="text-sm font-semibold text-[#151c27]">{title}</p>
                  <p className="text-xs text-[#404944] mt-0.5 leading-relaxed">{desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex justify-between items-center border-t border-[#dce2f3] pt-6">
          <button onClick={() => setStep(3)} className="text-sm text-[#404944] hover:text-[#003527] transition-colors flex items-center gap-2 px-4 py-3 rounded-lg hover:bg-[#e7eefe]">← Back</button>
          <button
